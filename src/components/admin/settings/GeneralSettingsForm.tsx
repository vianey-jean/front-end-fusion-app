...
const GeneralSettingsForm: React.FC<GeneralSettingsFormProps> = ({
  ...
  // Mutation pour mettre à jour la maintenance
  const maintenanceMutation = useMutation({
    mutationFn: maintenanceAPI.updateMaintenanceStatus,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['maintenance'] });
      toast.success('Mode maintenance mis à jour avec succès');
      // Recharger la page après 1 seconde pour appliquer le mode maintenance
      setTimeout(() => {
  ...
  const onSubmit = (data: GeneralSettings) => {
    // Si le mode maintenance a changé, mettre à jour séparément
    if (data.maintenanceMode !== maintenanceData?.maintenance || 
        data.maintenanceMessage !== maintenanceData?.description) {
      maintenanceMutation.mutate({
        maintenance: data.maintenanceMode,
        description: data.maintenanceMessage || 'Site en maintenance. Nous reviendrons bientôt !'
      });
    }
  ...
  // Mettre à jour les valeurs du formulaire avec les données de maintenance
  React.useEffect(() => {
    if (maintenanceData) {
      form.setValue('maintenanceMode', maintenanceData.maintenance);
      form.setValue('maintenanceMessage', maintenanceData.description);
    }
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex justify-end">
              <Button type="submit" disabled={isLoading || maintenanceMutation.isPending}>
