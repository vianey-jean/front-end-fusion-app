...
class NotificationService {
  private getNotificationStyle(type: NotificationType) {
    switch (type) {
      case 'success':
        return {
          className: "bg-gradient-to-r from-emerald-500 to-green-500 text-white border-0 fixed top-4 left-1/2 transform -translate-x-1/2 z-[100] shadow-2xl backdrop-blur-sm rounded-xl min-w-80 max-w-md",
          variant: 'default' as const
        };
      case 'error':
        return {
          className: "bg-gradient-to-r from-red-500 to-rose-500 text-white border-0 fixed top-4 left-1/2 transform -translate-x-1/2 z-[100] shadow-2xl backdrop-blur-sm rounded-xl min-w-80 max-w-md",
          variant: 'destructive' as const
        };
      case 'warning':
        return {
          className: "bg-gradient-to-r from-amber-500 to-orange-500 text-white border-0 fixed top-4 left-1/2 transform -translate-x-1/2 z-[100] shadow-2xl backdrop-blur-sm rounded-xl min-w-80 max-w-md",
          variant: 'default' as const
        };
      case 'info':
      default:
        return {
          className: "bg-gradient-to-r from-blue-500 to-indigo-500 text-white border-0 fixed top-4 left-1/2 transform -translate-x-1/2 z-[100] shadow-2xl backdrop-blur-sm rounded-xl min-w-80 max-w-md",
          variant: 'default' as const
        };
    }
  }
  ...
  show({ title, description, type, duration = 5000 }: NotificationOptions) {
    ...
    // Ajouter des icônes et styles selon le type
    ...
    const enhancedTitle = `${icon} ${title}`;
  ...
  // Messages spécifiques pour l'e-commerce avec emojis et styles améliorés
  ...
  removeFromCart(productName: string) {
    this.info("🗑️ Produit retiré", `${productName} a été retiré de votre panier`);
  }

  addToFavorites(productName: string) {
    this.success("❤️ Favoris", `${productName} ajouté à vos favoris`);
  }
